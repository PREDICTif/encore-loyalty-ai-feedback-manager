...
export default function ProfileLoader({ configuration, onConfigurationChange }: ProfileLoaderProps) {
  const [selectedRestaurantProfile, setSelectedRestaurantProfile] = useState<string>("");
  const [selectedCustomerProfile, setSelectedCustomerProfile] = useState<string>("");
  const [currentRestaurantId, setCurrentRestaurantId] = useState<string>("bistro-casual");
  ...
  const loadRestaurantMutation = useMutation({
    mutationFn: async (profileId: string) => {
      const response = await apiRequest("POST", "/api/load-restaurant-profile", { profileId });
  ...
  const loadCustomerMutation = useMutation({
    mutationFn: async ({ restaurantId, profileId }: { restaurantId: string; profileId: string }) => {
      const response = await apiRequest("POST", "/api/load-customer-profile", { restaurantId, profileId });
      return response.json();
    },
    ...
    <Card className="shadow-sm border-slate-200 mb-6">
      <CardHeader className="pb-4">
        <CardTitle className="text-lg font-semibold text-slate-800 flex items-center">
          <Upload className="text-blue-500 mr-2" size={18} />
          Profile Manager
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-medium text-slate-700">Restaurant Profiles</h3>
          </div>
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={!selectedRestaurantProfile || loadRestaurantMutation.isPending}
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-medium text-slate-700">Customer Profiles</h3>
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={!selectedCustomerProfile || loadCustomerMutation.isPending}
